...
def dirReduc(route):
    direction_dict = {"NORTH": 10, "SOUTH": -10, "EAST": -5, "WEST": 5}
    result = []
    direction_index = 0
    while direction_index < len(route):
        check_result(result, direction_dict, route[direction_index])
        direction_index += 1
    return result


def check_result(result: list, direction_dict, direction):
    if len(result) == 0:
        result.append(direction)
    elif direction_dict[result[-1]] + direction_dict[direction] == 0:
        result.pop(-1)
    else:
        result.append(direction)
